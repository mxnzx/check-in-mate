    ...
    >
      ...
      <div style="margin-top: auto">
        <div class="row m-2">
          <div class="col">
            <b-button
              id="plan-pick-list-add-btn"
              size="sm"
              variant="light"
              @click="addListDataByDate"
              >완료</b-button
            >
          </div>
          <div class="col">
            <b-button
              id="plan-pick-list-delete-btn"
              size="sm"
              variant="light"
              @click="deleteAll"
              >Reset</b-button
...
export default {
  ...
  methods: {
    ...
    addListDataByDate() {
      ...
      this.pickList = this.pickList.filter(
        (item) => item.dayIdx !== this.dayIdx
      ); // 현재 dayIdx에 해당하지 않는 항목만 남기기
